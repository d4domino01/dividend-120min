...
with tab1:
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.container(border=True).metric("Total Value", f"${df['Value'].sum() + safe_float(st.session_state.cash):,.0f}")
    with c2:
        st.container(border=True).metric("Monthly Income", f"${(df['Weekly Income'].sum()*52/12):,.0f}")

    c3, c4 = st.columns(2)
    with c3:
        st.container(border=True).metric("Annual Income", f"${(df['Weekly Income'].sum()*52):,.0f}")
    with c4:
        st.container(border=True).metric("Market", market)
...
st.caption("v30 • Stable mobile UI • 2-column KPI cards • Native ETF cards • Tabs intact • No features removed")