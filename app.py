...
if len(mom_rows) == 0:
    ...
    mom_df["Momentum"] = pd.to_numeric(mom_df["Momentum"], errors="coerce")
    mom_df["Volatility"] = pd.to_numeric(mom_df["Volatility"], errors="coerce")
    ...
    mom_df["Momentum"] = mom_df["Momentum"].apply(lambda x: f"{x:.2%}" if pd.notna(x) else "—")
...
st.caption("Momentum uses recent intraday candles with numeric coercion to prevent crashes.")