...
with tabs[1]:
    ...
    for t in etf_list:
        ...
        if hist:
            last_date = hist[-1][0]
            last_date_py = pd.to_datetime(last_date).to_pydatetime().replace(tzinfo=None)
...
st.caption("v3.10.6 • Dividend monitor added under Risk • timezone-safe • no removals")