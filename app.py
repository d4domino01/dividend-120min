...
st.set_page_config(page_title="Income Strategy Engine v4.4", layout="centered")
st.title("🔥 Income Strategy Engine v4.4")
...
# ==================================================
# 🔝 TOP WARNING BLOCK (FIRST THING USER SEES)
# ==================================================

dd = get_market_drawdown()

growth_list = [e["ticker"] for e in st.session_state.etfs if e["type"]=="Growth"]
income_list = [e["ticker"] for e in st.session_state.etfs if e["type"]=="Income"]
...
if dd is None or not isinstance(dd,(int,float)) or not np.isfinite(dd):
    ...
    if dd <= -0.20:
        st.error(
            "🔴 **CRASH MODE**\n\n"
            "Market is down more than 20%.\n\n"
            "👉 Consider trimming **Income ETFs** and rotating into:\n\n"
            f"**{', '.join(growth_list) if growth_list else 'Add Growth ETFs first'}**"
        )
        ...
        st.warning(
            "🟡 **DEFENSIVE MODE**\n\n"
            "Market is weakening.\n\n"
            "👉 Shift new contributions toward:\n\n"
            f"**{', '.join(growth_list) if growth_list else 'Add Growth ETFs first'}**"
        )
        ...
        st.success(
            "🟢 **NORMAL MODE**\n\n"
            "Income strategy operating normally.\n\n"
            "👉 Continue reinvesting into income ETFs."
        )
...
for e in st.session_state.etfs:
    ...
    rows.append([e["ticker"], e["type"], e["shares"], price, val, m_inc * e["shares"]])
